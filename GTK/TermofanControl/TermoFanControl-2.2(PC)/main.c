...
int main (int argc, char *argv[]){
    ...
    btnF1 = GTK_WIDGET(gtk_builder_get_object(builder, "btnF1"));
    btnF2 = GTK_WIDGET(gtk_builder_get_object(builder, "btnF2"));
    ...
    SpinTemp  = GTK_WIDGET(gtk_builder_get_object(builder, "SpinTemp"));


        //gtk_builder_connect_signals(builder, status);    // note: second parameter is not NULL
    
    g_signal_connect (btnF1, "clicked", G_CALLBACK (Press_F1), &comportdata);
    g_signal_connect (btnF2, "clicked", G_CALLBACK (Press_F2), NULL);
